document.addEventListener('DOMContentLoaded', () => {
    ...
    const profesorCardsContainer = document.getElementById('profesor-cards-container');

    // Validación inicial
    if (!menuToggle || !menuContainer || !profesorCardsContainer) {
        console.error('Error: Elementos del menú o contenedor de tarjetas de profesor no encontrados');
        return;
    }
    ...
    // Función para cargar profesores dinámicamente
    function loadProfesores() {
        fetch('../api/get_profesores.php')
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    profesorCardsContainer.innerHTML = ''; // Limpiar contenido existente
                    data.data.forEach(profesor => {
                        const card = `
                            <div class="card">
                                <img src="../multimedia/administrador/profesor.png" alt="Profesor">
                                <div class="card-content">
                                    <h3>Prof: ${profesor.nombres} ${profesor.apellidos}</h3>
                        ...
                        `;
                        profesorCardsContainer.innerHTML += card;
                    });
                } else {
                    console.error('Error al cargar profesores:', data.message);
                    profesorCardsContainer.innerHTML = '<p>Error al cargar los profesores.</p>';
                }
            })
            .catch(error => {
                console.error('Error de red al cargar profesores:', error);
                profesorCardsContainer.innerHTML = '<p>Error de conexión al cargar los profesores.</p>';
            });
    }
