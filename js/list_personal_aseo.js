document.addEventListener('DOMContentLoaded', () => {
    ...
    const personalAseoCardsContainer = document.getElementById('personal-aseo-cards-container');

    // Validación inicial
    if (!menuToggle || !menuContainer || !personalAseoCardsContainer) {
        console.error('Error: Elementos del menú o contenedor de tarjetas de personal de aseo no encontrados');
        return;
    }
    ...
    // Función para cargar personal de aseo dinámicamente
    function loadPersonalAseo() {
        fetch('../api/get_personal_aseo.php')
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    personalAseoCardsContainer.innerHTML = ''; // Limpiar contenido existente
                    data.data.forEach(personal => {
                        const card = `
                            <div class="card">
                                <img src="../multimedia/administrador/aseo.png" alt="Personal de Aseo">
                        ...
                        `;
                        personalAseoCardsContainer.innerHTML += card;
                    });
                } else {
                    console.error('Error al cargar personal de aseo:', data.message);
                    personalAseoCardsContainer.innerHTML = '<p>Error al cargar el personal de aseo.</p>';
                }
            })
            .catch(error => {
                console.error('Error de red al cargar personal de aseo:', error);
                personalAseoCardsContainer.innerHTML = '<p>Error de conexión al cargar el personal de aseo.</p>';
            });
    }

    // Cargar personal de aseo al cargar la página
    loadPersonalAseo();
});
