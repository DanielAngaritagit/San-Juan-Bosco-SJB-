document.addEventListener('DOMContentLoaded', () => {
    ...
    const bibliotecariosCardsContainer = document.getElementById('bibliotecarios-cards-container');

    // Validación inicial
    if (!menuToggle || !menuContainer || !bibliotecariosCardsContainer) {
        console.error('Error: Elementos del menú o contenedor de tarjetas de bibliotecarios no encontrados');
        return;
    }
    ...
    // Función para cargar bibliotecarios dinámicamente
    function loadBibliotecarios() {
        fetch('../api/get_bibliotecarios.php')
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    bibliotecariosCardsContainer.innerHTML = ''; // Limpiar contenido existente
                    data.data.forEach(personal => {
                        const card = `
                            <div class="card">
                                <img src="../multimedia/administrador/bibliotecario.png" alt="Bibliotecario">
                        ...
                        `;
                        bibliotecariosCardsContainer.innerHTML += card;
                    });
                } else {
                    console.error('Error al cargar bibliotecarios:', data.message);
                    bibliotecariosCardsContainer.innerHTML = '<p>Error al cargar los bibliotecarios.</p>';
                }
            })
            .catch(error => {
                console.error('Error de red al cargar bibliotecarios:', error);
                bibliotecariosCardsContainer.innerHTML = '<p>Error de conexión al cargar los bibliotecarios.</p>';
            });
    }

    // Cargar bibliotecarios al cargar la página
    loadBibliotecarios();
});
