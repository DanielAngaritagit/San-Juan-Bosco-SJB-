document.addEventListener('DOMContentLoaded', () => {
    ...
    const recepcionistasCardsContainer = document.getElementById('recepcionistas-cards-container');

    // Validación inicial
    if (!menuToggle || !menuContainer || !recepcionistasCardsContainer) {
        console.error('Error: Elementos del menú o contenedor de tarjetas de recepcionistas no encontrados');
        return;
    }
    ...
    // Función para cargar recepcionistas dinámicamente
    function loadRecepcionistas() {
        fetch('../api/get_recepcionistas.php')
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    recepcionistasCardsContainer.innerHTML = ''; // Limpiar contenido existente
                    data.data.forEach(personal => {
                        const card = `
                            <div class="card">
                                <img src="../multimedia/administrador/recepcionista.png" alt="Recepcionista">
                        ...
                        `;
                        recepcionistasCardsContainer.innerHTML += card;
                    });
                } else {
                    console.error('Error al cargar recepcionistas:', data.message);
                    recepcionistasCardsContainer.innerHTML = '<p>Error al cargar los recepcionistas.</p>';
                }
            })
            .catch(error => {
                console.error('Error de red al cargar recepcionistas:', error);
                recepcionistasCardsContainer.innerHTML = '<p>Error de conexión al cargar los recepcionistas.</p>';
            });
    }

    // Cargar recepcionistas al cargar la página
    loadRecepcionistas();
});
