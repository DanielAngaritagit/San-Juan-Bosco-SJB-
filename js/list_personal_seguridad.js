document.addEventListener('DOMContentLoaded', () => {
    ...
    const personalSeguridadCardsContainer = document.getElementById('personal-seguridad-cards-container');

    // Validación inicial
    if (!menuToggle || !menuContainer || !personalSeguridadCardsContainer) {
        console.error('Error: Elementos del menú o contenedor de tarjetas de personal de seguridad no encontrados');
        return;
    }
    ...
    // Función para cargar personal de seguridad dinámicamente
    function loadPersonalSeguridad() {
        fetch('../api/get_personal_seguridad.php')
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    personalSeguridadCardsContainer.innerHTML = ''; // Limpiar contenido existente
                    data.data.forEach(personal => {
                        const card = `
                            <div class="card">
                                <img src="../multimedia/administrador/seguridad.png" alt="Personal de Seguridad">
                        ...
                        `;
                        personalSeguridadCardsContainer.innerHTML += card;
                    });
                } else {
                    console.error('Error al cargar personal de seguridad:', data.message);
                    personalSeguridadCardsContainer.innerHTML = '<p>Error al cargar el personal de seguridad.</p>';
                }
            })
            .catch(error => {
                console.error('Error de red al cargar personal de seguridad:', error);
                personalSeguridadCardsContainer.innerHTML = '<p>Error de conexión al cargar el personal de seguridad.</p>';
            });
    }

    // Cargar personal de seguridad al cargar la página
    loadPersonalSeguridad();
});
