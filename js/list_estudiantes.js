document.addEventListener('DOMContentLoaded', () => {
    ...
    const estudianteCardsContainer = document.getElementById('estudiante-cards-container');

    // Validación inicial
    if (!menuToggle || !menuContainer || !estudianteCardsContainer) {
        console.error('Error: Elementos del menú o contenedor de tarjetas de estudiante no encontrados');
        return;
    }
    ...
    function loadEstudiantes() {
        fetch('../api/get_estudiantes.php')
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    estudianteCardsContainer.innerHTML = ''; // Limpiar contenido existente
                    data.data.forEach(estudiante => {
                        const card = `
                            <div class="card">
                                <img src="../multimedia/administrador/estudiante.png" alt="Estudiante">
                                <div class="card-content">
                                    <h3>Estudiante: ${estudiante.nombres} ${estudiante.apellidos}</h3>
                                    <p>Grado: ${estudiante.grado_numero || 'N/A'}${estudiante.letra_seccion || ''}</p>
                                    <button class="ver-mas" onclick="mostrarDetallesModal(
                                        '${estudiante.nombres}',
                                        '${estudiante.apellidos}',
                                        '${estudiante.telefono || 'N/A'}',
                                        'estudiante.png',
                                        '${estudiante.grado_numero || 'N/A'}${estudiante.letra_seccion || ''}'
                                    )">Ver más</button>
                                </div>
                            </div>
                        `;
                        estudianteCardsContainer.innerHTML += card;
                    });
                } else {
                    console.error('Error al cargar estudiantes:', data.message);
                    estudianteCardsContainer.innerHTML = '<p>Error al cargar los estudiantes.</p>';
                }
            })
            .catch(error => {
                console.error('Error de red al cargar estudiantes:', error);
                estudianteCardsContainer.innerHTML = '<p>Error de conexión al cargar los estudiantes.</p>';
            });
    }
...
document.addEventListener('DOMContentLoaded', function() {
    // ========== Modal de Detalles ==========
    function mostrarDetallesModal(nombres, apellidos, telefono, imagen, grado) {
        ...
        document.getElementById('modalGrado').textContent = grado;
