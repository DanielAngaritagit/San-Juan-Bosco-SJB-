...
echo "<!DOCTYPE html><html lang=\"es\"><head><meta charset=\"UTF-8\"><title>Fix Matrícula Sequence</title></head><body>";

echo "<h1>Herramienta para Corregir Secuencia de Matrícula</h1>";
...
try {
    ...
    // Obtener el máximo id_matricula actual
    $stmt_max_id = $pdo->query("SELECT MAX(id_matricula) FROM tab_matriculas");
    ...
    $stmt_setval = $pdo->prepare("SELECT setval('tab_matriculas_id_matricula_seq', :new_val, false)");
    $stmt_setval->bindParam(':new_val', $new_sequence_value, PDO::PARAM_INT);
    $stmt_setval->execute();

    $pdo->commit();

    echo "<p style=\"color: green;\">Secuencia 'tab_matriculas_id_matricula_seq' actualizada exitosamente a: <strong>" . $new_sequence_value . "</strong></p>";
